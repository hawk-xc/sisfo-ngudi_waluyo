<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="flex flex-col gap-4 p-4 mb-4 bg-white shadow-sm sm:rounded-lg md:flex-row md:items-center">
                <ul class="flex items-center justify-between flex-1 menu bg-base-200 lg:menu-horizontal rounded-box">
                    <div class="flex flex-row items-center gap-3">
                        ...
                        <li class="dropdown dropdown-hover dropdown-end">
                            ...
                            <ul tabindex="0"
                                ...
                                <li>
                                    <a href="{{ route('kader.index', ['sort' => 'asc']) }}"
                                        class="flex items-center gap-2">
                                        <i class="ri-sort-asc"></i> Sort Asc
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('kader.index', ['sort' => 'desc']) }}"
                                        class="flex items-center gap-2">
                    ...
                    <label class="input">
                        ...
                        <input id="search" type="search" class="grow" placeholder="Cari Data Kader" />
                    </label>
                </ul>
            </div>
            ...
            <div class="overflow-x-auto bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <table
                        class="w-full text-sm text-left text-gray-500 rtl:text-right  {{ $kaders->isEmpty() ? 'hidden' : '' }}">
                        {{-- {{ $pemeriksaan->isEmpty() ? 'hidden' : '' }} --}}
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr class="grid grid-cols-12">
                                <th class="col-span-1 px-6 py-3">ID</th>
                                <th class="col-span-4 px-6 py-3">Nama</th>
                                <th class="col-span-4 px-6 py-3">Email</th>
                            ...
                            @forelse ($kaders as $kader)
                                <tr class="grid grid-cols-12 bg-white border-b border-gray-200">
                                    ...
                                    <td class="flex col-span-2 px-2 py-4 join">
                                        <a href="{{ route('kader.show', $kader->id) }}"
                                        ...
                                        @role('admin')
                                            <form id="delete-form-{{ $kader->id }}"
                                                action="{{ route('kader.destroy', $kader->id) }}" method="POST"
                                                class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            ...
                                            <button class="btn join-item btn-sm btn-error btn-outline"
                                                onclick="confirmDelete('{{ $kader->id }}')">
    ...
    <script>
        document.getElementById('search').addEventListener('input', function(e) {
            ...
            document.getElementById('kaderTable').innerHTML =
                '<tr><td colspan="6" class="py-4 text-center">Mencari data...</td></tr>';

            fetch(`{{ route('kader.index') }}?search=${query}`, {
                    headers: {
                ...
                .then(data => {
                    document.getElementById('kaderTable').innerHTML = data.html;
