    ...
    <div :class="sidebarOpen ? 'w-64' : 'w-0'"
        ...
        <ul class="w-64 min-h-full p-4 space-y-2 menu bg-base-100 text-base-content" x-show="sidebarOpen">
            <li class="{{ request()->routeIs('dashboard') ? 'bg-slate-200' : '' }} rounded-md"><a
                    ...
                    Halaman
                    Utama</a></li>
            <li class="{{ request()->routeIs('lansia.*') ? 'bg-slate-200' : '' }} rounded-md">
                <a class="{{ request()->routeIs('lansia') ? 'font-semibold' : '' }}"
                    href="{{ route('lansia.index') }}"><i class="ri-group-line"></i> Data Lansia</a>
            </li>

            @role('admin')
                <li class="{{ request()->routeIs('kader.*') ? 'bg-slate-200' : '' }} rounded-md">
            ...
            @role('kader|admin')
                <li class="{{ request()->routeIs('pj.*') ? 'bg-slate-200' : '' }} rounded-md">
                    <a class="{{ request()->routeIs('pj') ? 'font-semibold' : '' }}" href="{{ route('pj.index') }}"><i
                            class="ri-user-settings-line"></i> Penanggung Jawab</a>
                </li>
            @endrole

            <li class="{{ request()->routeIs('pemeriksaan.*') ? 'bg-slate-200' : '' }} rounded-md">
